# -*- coding: utf-8 -*-
"""
This file aims to include all parameters used in BREVO model
...
def Initialize():
    ...
    return  C_ini, mileage, Q_acc, runtime, TimStp_Age, TimStp_RunOff, TimStp_RunOn, T_acharg, T_b, T_c, usetime
...
def VehicleInfo():
    ...
    m_v = 1800.  # vehicle gross weight, unit: kg
    ...
    """
    f_pt_max = 0.97  # powertrain efficiency at rated load (energy from bat to wheel)
    f_pt_min = 0.6  # powertrain efficiency at very light load
    f_acc = 0.98  # accessory efficiency (electronics etc)

    """
    Fixed powertrain losses: representing motor iron loss, mechanical friction,
    cooling system, inverter standby power, and control system power at zero torque.
    This is the baseline power consumption when motor is rotating.
    Unit: kW
    """
    P_pt_fixed = 0.2  # Fixed powertrain loss, unit: kW
    ...
    return A_d, c_d, c_r, f_acc, f_pt_max, f_pt_min, f_rbs, m_v, grav, rho_air, sigma, theta, v_wind, P_pt_fixed
...
def Lookup_OCVandR():
    ...
    SOC_OCVy = [327.36, 343.68, 353.28, 390.72, 403.20]
